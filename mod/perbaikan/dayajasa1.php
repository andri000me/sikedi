...
?>
	...
	<div class="table-header">
		...
		<form class='form-search' method='GET'>
			...
			<select class='span2 chosen-select' id='bln' name='bln' data-placeholder='Pilih Bulan'>";
				$qpr = mysql_query("SELECT DISTINCT bln as mnth FROM dayajasa ORDER BY bln ASC");
			...
			<select class='span1 chosen-select' id='thn' name='thn' data-placeholder='Pilih Tahun'>";
				$qpr = mysql_query("SELECT DISTINCT thn as year FROM dayajasa ORDER BY year DESC");
	...
	<?php
		...
		$l1 = mysql_fetch_array(mysql_query("SELECT sum(bbm) as jbensin FROM rep_kendi WHERE kdkab='$d[kdkab]' AND jbbm='1' AND MONTH(tglrep)='$d[bln]'"));
		$jbensin  = $l1['jbensin'];
		$njbensin = number_format ($jbensin, 0, ',', ',');

		$l2 = mysql_fetch_array(mysql_query("SELECT sum(bbm) as jpertalite FROM rep_kendi WHERE kdkab='$d[kdkab]' AND jbbm='2' AND MONTH(tglrep)='$d[bln]'"));
		$jpertalite  = $l2['jpertalite'];
		$njpertalite = number_format ($jpertalite, 0, ',', ',');

		$l3 = mysql_fetch_array(mysql_query("SELECT sum(bbm) as jpertamax FROM rep_kendi WHERE kdkab='$d[kdkab]' AND jbbm='3' AND MONTH(tglrep)='$d[bln]'"));
		$jpertamax  = $l3['jpertamax'];
		$njpertamax = number_format ($jpertamax, 0, ',', ',');

		$l4 = mysql_fetch_array(mysql_query("SELECT sum(bbm) as jsolar FROM rep_kendi WHERE kdkab='$d[kdkab]' AND jbbm='4' AND MONTH(tglrep)='$d[bln]'"));
		$jsolar  = $l4['jsolar'];
		$njsolar = number_format ($jsolar, 0, ',', ',');



		$t1 = mysql_fetch_array(mysql_query("SELECT sum(biaya) as bensin FROM rep_kendi WHERE kdkab='$d[kdkab]' AND jbbm='1' AND MONTH(tglrep)='$d[bln]'"));
		$bensin  = $t1['bensin'];
		$nbensin = number_format ($bensin, 0, ',', ',');

		$t2 = mysql_fetch_array(mysql_query("SELECT sum(biaya) as pertalite FROM rep_kendi WHERE kdkab='$d[kdkab]' AND jbbm='2' AND MONTH(tglrep)='$d[bln]'"));
		$pertalite  = $t2['pertalite'];
		$npertalite = number_format ($pertalite, 0, ',', ',');

		$t3 = mysql_fetch_array(mysql_query("SELECT sum(biaya) as pertamax FROM rep_kendi WHERE kdkab='$d[kdkab]' AND jbbm='3' AND MONTH(tglrep)='$d[bln]'"));
		$pertamax  = $t3['pertamax'];
		$npertamax = number_format ($pertamax, 0, ',', ',');

		$t4 = mysql_fetch_array(mysql_query("SELECT sum(biaya) as solar FROM rep_kendi WHERE kdkab='$d[kdkab]' AND jbbm='4' AND MONTH(tglrep)='$d[bln]'"));
		$solar  = $t4['solar'];
