...
$ru = getData("*","ms_ruangan","idruang='$rid'");
...
<div class="span7">
	...
	<div class="table-header">
	   INVENTARIS RUANGAN <?php echo $ru['rKode'];?> - <?php echo $ru['rNama'];?>  [<?php echo $rj;?>]
	</div>
	<div class="row-fluid">
			...
			<div class="widget-main no-padding center">
					...
					<fieldset data-rel="tooltip" data-original-title="Hanya Inventaris Yang Tersedia Yang Dapat Ditambahkan">
						<div class="controls">
							...
							$qpr = mysql_query("SELECT a.kdkab,a.bkode,a.bjenis,a.bmerek,a.bserial
															FROM barang a WHERE a.kdkab = '$kd' AND a.bkode NOT IN (SELECT pInv FROM penempatan)");
				...
				<?php
					if (isset($_POST['simpan'])){
						...
						$sukses = 0;
						foreach ($inv as $x => $y) {
							$q = mysql_query("INSERT INTO penempatan (kdkab,pInv,pRuang)
											  				VALUES ('$kd','$y','$rkode')");
							if ($q){
								$sukses++;
							}
