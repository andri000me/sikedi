...
<div class="widget-main">
	...
	<form method="POST" enctype="multipart/form-data" class="form-horizontal">
		<div class="control-group">
			<label class="control-label" for="uid">NIP</label>
			<div class="controls">
				<input type="text" class="input-medium" id="uid" name="uid" value=""  required>
			</div>
		</div>
		...
		<div class="control-group">
			...
			<div class="controls">
				<select class="span3 chosen-select" name="kdkab" id="kdkab" placeholder="Pilih Satker">
				<option>-- Pilih Satker --</option>				
					<?php
					if ($kodelvl =='1200') {
						$qsp = mysql_query("SELECT * FROM kdkab ORDER BY id_kabkot ASC");
					} else {
						$qsp = mysql_query("SELECT * FROM kdkab WHERE id_kabkot ='$kodelvl' ORDER BY id_kabkot ASC");

					}
						while ($s=mysql_fetch_array($qsp)) {
							if ($e['kodelvl']==$s['id_kabkot']){
		...
		<div class="control-group">
			...
			<div class="controls">
				<select class="span2" name="lvl" id="lvl" placeholder="Pilih Level">
				<option>-- Pilih Level --</option>
					<?php
						$qsp = mysql_query("SELECT * FROM _level");
						while ($s=mysql_fetch_array($qsp)) {
							if ($e['uLevel']==$s['id']){
	...
	<?php
		if (isset($_POST['simpan'])){
			...
			if (!empty($lokasi_file)){
				...
				$q = mysql_query("INSERT INTO user (uId,uUname,uNama,kdkab,uLevel,uTelp,
			                                        uEmail,uPass,uFoto)
												 VALUES('$_POST[uid]','$_POST[username]','$_POST[nama]','$_POST[kdkab]','$_POST[lvl]','$_POST[telp]',
...
<div class="widget-main">
	...
	<form method="POST" enctype="multipart/form-data" class="form-horizontal">
		<div class="control-group">
			...
			<div class="controls">
				<input type="text" class="input-medium" id="uid" name="uid" value="<?php echo $uid;?>" required>
			</div>
		</div>
		<div class="control-group">
			<label class="control-label" for="kdkab">Kab/Kota</label>
	...
	<?php
		if (isset($_POST['simpan'])){
			...
			if (!empty($lokasi_file)){
				...
				$q = mysql_query("UPDATE user SET uId='$_POST[uid]',uNama='$_POST[nama]',kdkab='$_POST[kdkab]',uLevel='$_POST[lvl]',uTelp='$_POST[telp]',uEmail='$_POST[email]',uFoto='$foto',
			                                     onUpdate=NOW()
			                                 WHERE uId='$_POST[uid]'
			                    ");
...
<div class="widget-main">
	...
	<form method="POST" enctype="multipart/form-data" class="form-horizontal">
		<div class="control-group">
			...
			<div class="controls">
				<input type="text" class="input-medium" id="uid" name="uid" value="<?php echo $e['uId'];?>" disabled>
			</div>
		</div>
		<div class="control-group">
			<label class="control-label" for="username">Username</label>
	...
	<?php
		if (isset($_POST['simpan'])){
			$pass = md5($_POST['password']);
			$q = mysql_query("UPDATE user SET  uId ='$_POST[uid]', 
			...
			if ($q){
			echo "<script>
			 		notifsukses('$pass Sukses','Data Telah Tersimpan..!!');
			  		setTimeout('window.location.href=\"media.php?page=$page\"', 1000)
			      </script>";
			}else{
			echo "<script>
...
?>
	...
	<tbody>
	 <?php
	    $qry = mysql_query("SELECT * FROM user WHERE kdkab='$kd'  ORDER BY onCreate DESC");
		while ($d = mysql_fetch_array($qry)){
	      $no++;
	      $ulvl = getValue("level","_level","id='$d[uLevel]'");
	      echo "
	      <tr>
	      <td class='center'>$no</td>
	      <td class='left'>$d[uId]</td>
	      <td>$d[kdkab]</td>
	      <td>$d[uNama]</td>
	      <td class='left'>$ulvl</td>
	      <td class='left'>$d[uTelp]</td>
	      <td>$d[uEmail]@bps.go.id</td>
	      <td class='left'>$d[uUname]</td>
	      <td class='center'>
